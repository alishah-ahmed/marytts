 ...
 * to receive/send data from/to server. To be able to use the functionality
 * provided by this class, all Mary clients should either: (i) extend this class
 * (Example: MaryHttpClient, MaryWebHttpClient) (ii) use an object of type
 * MaryHttpForm or of a derived class (Example: MaryGUIClient)
 * 
...
public class MaryFormData {
	...
	public MaryFormData(Address serverAddress, String versionIn,
			String voicesIn, String dataTypesIn, String audioFileFormatTypesIn,
			String audioEffectHelpTextLineBreakIn, String defaultAudioEffects,
			Vector<String> defaultVoiceExampleTexts) {
		this(serverAddress, null, versionIn, voicesIn, dataTypesIn,
				audioFileFormatTypesIn, audioEffectHelpTextLineBreakIn,
				defaultAudioEffects, defaultVoiceExampleTexts);
	}

	public MaryFormData(Address serverAddress,
			Map<String, String> keyValuePairsIn, String versionIn,
			String voicesIn, String dataTypesIn, String audioFileFormatTypesIn,
			String audioEffectHelpTextLineBreakIn, String defaultAudioEffects,
			Vector<String> defaultVoiceExampleTexts) {
		init(serverAddress, keyValuePairsIn, versionIn, voicesIn, dataTypesIn,
				audioFileFormatTypesIn, audioEffectHelpTextLineBreakIn,
				defaultAudioEffects, defaultVoiceExampleTexts);
	}
	...
	public void toServerVersionInfo(String info) {
		...
		if (serverVersionInfo != null) {
			...
			if (parts[0].equals("Mary") && parts[1].equals("TTS")
					&& parts[2].equals("server") && parts.length >= 4) {
	...
	public void toVoices(String info) {
		...
		if (voiceArray != null && voiceArray.size() > 0) {
			...
			for (int i = 0; i < voiceArray.size(); i++) {
				...
				String name = currentVoice.has("name") ? currentVoice
						.get("name").toString().replace("\"", "") : "";
				String localeString = currentVoice.has("locale") ? currentVoice
						.get("locale").toString().replace("\"", "") : "";
				Locale locale = string2locale(localeString);
				// locale = string2locale("en-US");
				assert locale != null;
				String gender = currentVoice.has("gender") ? currentVoice
						.get("gender").toString().replace("\"", "") : "";
				MaryClient.Voice voice = null;
				if (!currentVoice.has("domain")) {
					voice = new MaryClient.Voice(name, locale, gender,
							"general");
				} else {
					String domain = currentVoice.get("domain").toString()
							.replace("\"", "");
					voice = new MaryClient.Voice(name, locale, gender, domain);
				}
				if (isServerVersionAtLeast("3.5.0")) {
					String synthesizerType = currentVoice.has("type") ? currentVoice
	...
	public void toDataTypes(String info) {
		...
		if (info != null && info.length() > 0) {
			...
			for (int i = 0; i < typeStrings.length; i++) {
				...
				MaryClient.DataType dt = new MaryClient.DataType(name,
						isInputType, isOutputType);
				allDataTypes.add(dt);
				if (dt.isInputType()) {
	...
	private void toSelections(Map<String, String> keyValuePairsIn,
			Vector<String> defaultVoiceExampleTexts) {
		...
		if (effectsBoxData == null) {
			 ...
			 * getAudioEffectHelpTextLineBreak(); if (audioEffects==null)
			 * getAudioEffects();
			 */
			effectsBoxData = new AudioEffectsBoxData(audioEffects);
		}
		...
		if (selected != null) {
			...
			if (outputDataTypes.get(outputTypeSelected).name()
					.contains("AUDIO"))
				isOutputText = false;
			else
				isOutputText = true;
			...
			if (allVoices.elementAt(voiceSelected).isLimitedDomain()) {
				...
				if (limitedDomainExampleTexts != null && selected != null) {
					for (i = 0; i < limitedDomainExampleTexts.size(); i++) {
						if (limitedDomainExampleTexts.get(i)
		...
		if (selected != null)
			...
			if (allVoices != null && allVoices.size() > 0
					&& inputDataTypes != null && inputDataTypes.size() > 0) {
				if (allVoices.elementAt(voiceSelected).isLimitedDomain()
						&& limitedDomainExampleTexts != null)
					inputText = limitedDomainExampleTexts
							.get(limitedDomainExampleTextSelected);
				else if (serverExampleTexts != null)
					inputText = serverExampleTexts.get(inputDataTypes.get(
							inputTypeSelected).name()
							+ " "
							+ allVoices.elementAt(voiceSelected).getLocale()
									.toString());
			}
		}
		//
		...
		if (selected != null) {
			...
			for (i = 0; i < audioFileFormatTypes.size(); i++) {
				...
				String typeName = audioFileFormatTypes.get(i).substring(
						spaceInd + 1);
		...
		for (i = 0; i < effectsBoxData.getTotalEffects(); i++) {
			currentEffectName = effectsBoxData.getControlData(i)
					.getEffectName();

			// Check box
			selected = keyValuePairs.get("effect_" + currentEffectName
					+ "_selected");
	...
	public void checkAndCorrectSelections() {
		audioFormatSelected = MathUtils.CheckLimits(audioFormatSelected, 0,
				audioFileFormatTypes.size() - 1);
		audioOutSelected = MathUtils.CheckLimits(audioOutSelected, 0,
				audioOutTypes.size() - 1);
		inputTypeSelected = MathUtils.CheckLimits(inputTypeSelected, 0,
				inputDataTypes.size() - 1);
		outputTypeSelected = MathUtils.CheckLimits(outputTypeSelected, 0,
				outputDataTypes.size() - 1);
		voiceSelected = MathUtils.CheckLimits(voiceSelected, 0,
				allVoices.size() - 1);
	}

	/**
	 * This helper method converts a string (e.g., "en_US") into a proper Locale
	 * object.
	 * 
	 * @param localeString
